// import { Users, TrendingUp, CheckCircle, Phone, Bell, AlertCircle } from 'lucide-react'
...
//         <p style={{ fontSize:11, fontWeight:600, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'0.05em' }}>{label}</p>
//         <div style={{ width:32, height:32, borderRadius:8, background:bg, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
//           {icon}
//         </div>
//       </div>
//       <p style={{ fontSize:26, fontWeight:700, color: urgent ? '#dc2626' : '#0f172a', lineHeight:1, marginBottom:4 }}>{value}</p>
...
//   const winRate = st.total > 0 ? Math.round(((st.byStatus['Closed']??0)/st.total)*100) : 0

//   const cards: CardProps[] = [
//     { label:'Total Leads',     value:st.total,                       sub:`+${st.thisMonth} this month`,    icon:<Users size={14} color="#4c6ef5" />,      accent:'ac-blue',   bg:'#eff4ff' },
//     { label:'New',             value:st.byStatus['New']??0,           sub:'Awaiting contact',               icon:<TrendingUp size={14} color="#0284c7" />,  accent:'ac-sky',    bg:'#f0f9ff', onClick:() => nav('/leads?status=New') },
//     { label:'Contacted',      value:st.byStatus['contacted']??0,    sub:'High intent',                    icon:<TrendingUp size={14} color="#7c3aed" />,  accent:'ac-violet', bg:'#f5f3ff', onClick:() => nav('/leads?status=Contacted') },
//     { label:'Closed Won',      value:st.byStatus['Closed']??0,        sub:`${winRate}% win rate`,           icon:<CheckCircle size={14} color="#16a34a" />, accent:'ac-green',  bg:'#f0fdf4', onClick:() => nav('/leads?status=Closed') },
//     { label:"Today's Follow-ups", value:st.todayFollowups,            sub:'Due today',                      icon:<Bell size={14} color="#d97706" />,        accent:'ac-amber',  bg:'#fffbeb', onClick:() => nav('/followups') },
//     { label:'Overdue',         value:st.overdueFollowups,             sub:'Need immediate attention',       icon:<AlertCircle size={14} color="#dc2626" />, accent:'ac-red',    bg:'#fef2f2', onClick:() => nav('/followups?tab=overdue'), urgent:st.overdueFollowups>0 },
//   ]
...
// ✅ FIXED: PascalCase status keys match normalized stats, accurate counts
