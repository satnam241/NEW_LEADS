...
// import { EXPORT_OPTIONS, exportLeads, fetchLeadsForExport, type ExportFilter, type ExportFormat } from './importExport'
...
// export default function ExportDropdown({ leads, allLeads }: Props) {
...
//   // ✅ FINAL EXPORT FUNCTION (SUPABASE BASED)
//   const handleExport = async (filter: ExportFilter) => {
//     try {
//       const data = await fetchLeadsForExport(filter)

//       if (!data || data.length === 0) {
//         toast.error(`No ${filter === 'all' ? '' : filter + ' '}leads to export`)
//         return
//       }

//       exportLeads(data, fmt)

//       toast.success(`Exporting ${data.length} leads as ${fmt.toUpperCase()}…`)
//       setOpen(false)
...
//         style={{ height: 36, fontSize: 13, gap: 5 }}
//         onClick={() => setOpen(o => !o)}
//       >
//         <Download size={13} />
//         Export
//         <ChevronDown size={12} style={{ transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }} />
//       </button>
