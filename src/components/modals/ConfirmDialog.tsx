...
export default function ConfirmDialog({ open, title, message, confirmLabel = 'Delete', onConfirm, onCancel, isLoading }: Props) {
  ...
  return (
    <div className="modal-backdrop" onClick={e => { if (e.target === e.currentTarget) onCancel() }}>
      <div className="background:'#3C3C3C' border:'1px solid rgba(255,255,255,.08)' rounded-2xl w-full max-w-sm border border-slate-200 p-6" style={{ animation: 'modalIn 200ms cubic-bezier(0.16,1,0.3,1) both' }}>
        <div className="flex items-start gap-3 mb-4">
          <div className="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center flex-shrink-0">
            <AlertTriangle size={16} className="text-red-500" />
          </div>
          <div>
            <h3 className="font-semibold text-slate-500 text-sm mb-1 color:'#94a3b8'">{title}</h3>
            <p className="text-xs text-slate-500 leading-relaxed">{message}</p>
          </div>
        </div>
        <div className="flex justify-end gap-2">
          <button className="color:'#fff' background:'#2a2d3e' text-xs py-1.5 px-3" onClick={onCancel}>Cancel</button>
