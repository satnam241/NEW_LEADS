...
// import { X, Upload, FileSpreadsheet, CheckCircle2, AlertCircle, Loader2, ChevronRight } from 'lucide-react'
// import { readSpreadsheet, type ImportResult } from './importExport'
...
// const ACCEPTED_LABEL = 'CSV, Excel (.xlsx/.xls), ODS, or plain text'
...
//     setImported(0); setFailed(0); setFileName('')
//     setDragover(false)
//   }
...
//     setStep('done')
//     if (ok > 0) toast.success(`${ok} lead${ok > 1 ? 's' : ''} imported!`)
//   }
...
//               {step === 'upload'    && 'Upload any spreadsheet file'}
//               {step === 'preview'   && `${result?.leads.length ?? 0} leads ready · ${result?.skipped ?? 0} rows skipped`}
//               {step === 'importing' && `Importing ${imported} of ${result?.leads.length ?? 0}…`}
//               {step === 'done'      && `Done! ${imported} imported · ${failed} failed`}
//             </p>
//           </div>
...
//               {/* Column guide */}
//               <div style={{ marginTop: 20, background: '#f8fafc', borderRadius: 10, padding: '14px 16px', border: '1px solid #f1f5f9' }}>
//                 <p style={{ fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 10 }}>📋 Supported columns (any order, any case)</p>
...
//                       <span style={{ fontSize: 11, fontWeight: 600, color: '#4c6ef5', background: '#eff4ff', padding: '2px 7px', borderRadius: 5 }}>{col}</span>
//                       <span style={{ fontSize: 11, color: '#94a3b8' }}>{hint}</span>
//                     </div>
//                   ))}
//                 </div>
//                 <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 10 }}>
//                   Missing columns will use smart defaults. Phone numbers are auto-formatted.
//                 </p>
//               </div>
//             </div>
//           )}
...
//             <div>
//               {/* File info */}
//               <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', background:'#f0fdf4', borderRadius:10, border:'1.5px solid #bbf7d0', marginBottom:16 }}>
//                 <FileSpreadsheet size={20} style={{ color:'#16a34a', flexShrink:0 }} />
//                 <div>
//                   <p style={{ fontSize:13, fontWeight:600, color:'#15803d' }}>{fileName}</p>
//                   <p style={{ fontSize:11, color:'#4ade80' }}>{result.total} rows read · {result.leads.length} valid · {result.skipped} skipped</p>
//                 </div>
...
//                           <th key={h} style={{ padding:'8px 12px', textAlign:'left', fontWeight:600, color:'#94a3b8', fontSize:10.5, textTransform:'uppercase', letterSpacing:'0.05em', whiteSpace:'nowrap', borderBottom:'1px solid #f1f5f9' }}>{h}</th>
//                         ))}
//                       </tr>
//                     </thead>
//                     <tbody>
...
//                           <td style={{ padding:'8px 12px', color:'#374151', fontWeight:500 }}>{l.name}</td>
//                           <td style={{ padding:'8px 12px', color:'#64748b', fontFamily:'monospace', fontSize:11 }}>{l.phone ?? '—'}</td>
...
//                     {result.skipped} row{result.skipped > 1 ? 's were' : ' was'} skipped because they had no name.
//                   </p>
//                 </div>
//               )}
//             </div>
...
//               <Loader2 size={32} className="animate-spin" style={{ color:'#4c6ef5', margin:'0 auto 16px' }} />
//               <p style={{ fontSize:14, fontWeight:600, color:'#374151', marginBottom:4 }}>Importing leads…</p>
//               <p style={{ fontSize:12, color:'#94a3b8', marginBottom:20 }}>{imported} of {result?.leads.length} completed</p>
//               <div className="progress-bar-wrap" style={{ maxWidth:320, margin:'0 auto' }}>
...
//               <p style={{ fontSize:16, fontWeight:600, color:'#15803d', marginBottom:6 }}>{imported} leads imported!</p>
//               {failed > 0 && <p style={{ fontSize:12, color:'#dc2626', marginBottom:4 }}>{failed} failed (duplicates or errors)</p>}
//               <p style={{ fontSize:12, color:'#94a3b8' }}>Your dashboard has been updated.</p>
//             </div>
//           )}
//         </div>
...
//               <button className="btn-primary" onClick={startImport} disabled={!result || result.leads.length === 0}>
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (res.leads.length === 0) {
        toast.error('Koi valid lead nahi mili. Column names check karein.')
        return
      }
      setResult(res)
      setStep('preview')
    } catch (err: any) {
      toast.error('File read nahi ho saki: ' + (err.message || 'Unknown error'))
    }
  }, [])
